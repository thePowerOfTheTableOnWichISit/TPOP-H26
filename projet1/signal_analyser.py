...
def analyze_pulses(voltages, sample_rate, low_threshold=0.5, high_threshold=0.5, window_size=15, min_samples=100):
    """
...
def analyze_positive_pulses(voltages, sample_rate, bridge_samples=1000, min_fall_samples=50):
    ...
    # --- STAGE 2: DYNAMIC THRESHOLDING ---
    v_bottom = np.percentile(bridged_v, 2)
    v_max = np.max(bridged_v)
    ...
    rise_thresh = v_bottom + (v_range * 0.15)
    fall_thresh = v_bottom + (v_range * 0.02)
...
def plot_pulse_comparison(time1, volt1, pulses1, time2, volt2, pulses2, title="Pulse Detection Comparison"):
    ...
    # --- TOP PLOT: Dataset 1 ---
    ax1.plot(time1[::step], volt1[::step], color='royalblue', alpha=0.7, label='Signal 1 (Raw)')
...
def analyze_pulse_deltas(robust_pulses, precision_pulses):
    ...
    for i, anchor in enumerate(robust_pulses):
        ...
        delta_results.append({
            "pulse_index": i,
            "base_detecteur_start": a_s,
            "distance_detecteur_start": prec_s,
            "start_delta_ms": start_delta * 1000,
            "end_delta_ms": end_delta * 1000,
            "duration_diff_ms": (prec_e - prec_s - (a_e - a_s)) * 1000
        })
    ...
    stats = {
        ...
        "std_dev_ms": df["start_delta_ms"].std()
    }
...
if __name__ == '__main__':
    for file_name in files:
        ...
        pulse_results1, smoothed_signal1 = analyze_pulses(volt, fs)
        ...
        if index in [4, 26]:
            ...
            pulse_results2, smoothed_signal2 = analyze_positive_pulses(volt_fixed2, fs)
            pulse_results2_aligned = fuse_pulses_nearest_neighbor(pulse_results1, pulse_results2)
        ...
        plot_complete_with_highlights(time2[::1000], volt_fixed2[::1000], pulse_results2, smoothed_signal=smoothed_signal2[::1000], file_name=file_name)

        #pulse_results = analyze_pulses(volt, low_threshold=0.5, high_threshold=2.0, sample_rate=fs)
        plot_pulse_comparison(time, volt, pulse_results1, time2, smoothed_signal2, pulse_results2)
        plot_pulse_comparison(time, volt, pulse_results1, time2, smoothed_signal2, pulse_results2_aligned)
